...
export async function GET(request: Request) {
  ...
  const endpoint = new URL(`${url}/rest/v1/regions`);
  ...
  if (!res.ok) return fail(502, "UPSTREAM_ERROR", "Failed to fetch regions", requestId);

  const regions = await res.json();
  return ok({ regions }, requestId);
}
...
export async function POST(request: Request) {
  ...
  const createRes = await fetch(`${url}/rest/v1/regions`, {
    method: "POST",
    headers: {
  ...
  if (!createRes.ok) return fail(502, "UPSTREAM_ERROR", "Failed to create region", requestId);

  const rows = (await createRes.json()) as unknown[];
  const region = Array.isArray(rows) ? rows[0] : rows;
  ...
  return new Response(JSON.stringify({ requestId, data: { region } }), {
    status: 201,
